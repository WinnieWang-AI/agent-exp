...
1. **构建 shot 时间轴表**：按 shot_order 逐 shot 累加 duration_seconds，输出每个 shot 的绝对时间范围 `{shot_id: [start, end]}`。写下这张表，后续步骤从表中查值，不再手算偏移。
2. **提取对白并查表**：从每个 shot 的 content 中提取对白文本，查时间轴表得到该 shot 的 [start, end]
3. **估算对白时长**：
   - 中文：每秒 3-4 个字
   - 英文：每秒 2-3 个单词
   - 最小 1.5 秒，最大不超过 shot 时长
4. **居中放置**：在该 shot 的 [start, end] 范围内居中
5. **自检**：字幕条目的时间顺序与 shot_order 一致，且无时间重叠
