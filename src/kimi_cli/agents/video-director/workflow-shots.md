...
1. 统计事件总数
2. 按事件的叙事权重分配时长预算：
   - **高潮/转折事件**（mood 紧张、冲突、惊讶）：分配较多时长（多个短 shot）
   - **过渡/铺垫事件**（mood 平静、日常）：分配较少时长（1-2 个 shot）
   - **开场/结尾事件**：适当留余量（fade_in/fade_out 需要时间）
3. 确保预算总和与目标时长大致匹配
...
3. **设计每个 shot**：
   ...
   **配乐（bgm）：**
   - 描述该 shot 的配乐情绪和风格方向（如"紧张的弦乐震音"、"轻柔的木吉他"）
   - 相邻 shot 配乐相同时可省略，表示延续
   - 配乐变化通常发生在情绪转折点（mood 变化、场景切换、高潮前后）
   - 无配乐的 shot 写 "silence" 或省略
   ...
   **时长（duration_seconds）：**
   ...
   - 日常/对话镜头：5-8 秒
   - 抒情/环境镜头：6-10 秒
   - 单个 shot 不超过 10 秒（AI 视频生成限制）

   **转场：**
   - 同一事件内的 shot 之间：cut
   - 相邻事件、同场景：cut
   - 跨场景/跨时间：dissolve 或 fade
...
11. **对白时长匹配**：含对白的 shot 时长足够容纳对白（粗略估算：中文每秒约 3-4 个字）
