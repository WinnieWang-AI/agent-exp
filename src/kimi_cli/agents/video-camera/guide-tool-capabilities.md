...
### Vidu Q3（viduq3-pro / viduq3-turbo）
...
### Vidu Q2（viduq2 / viduq2-pro / viduq2-turbo）

| 模式 | 视频 | 音频 | 时长 |
|------|------|------|------|
| text_to_video | 是 | 否 | 1-10s |
| image_to_video | 是 | 否 | 1-10s |
| reference_to_video | 是 | 否 | 1-10s |
| reference_to_video (subjects) | 是 | 是 | 1-10s |
| start-end2video | 是 | 否 | 1-10s |

- 仅 subjects 模式（Ref2V-Audio）有音频
- 其他模式输出静音视频
...
1. **优先 GenerateVideoSync**：如果 provider 支持音频（如 Vidu Q3），所有声音（对白 + 音效 + 环境音）交给视频 API，通过 prompt Sound 段描述
