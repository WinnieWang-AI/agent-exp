...
- **Prompt materials** for each shot: all active appearances, minds, location states, prop states, audio states, and current relationships — extracted via graph queries so the LLM does not need to cross-reference `*_active_during` maps manually.
...
- **Camera language**: each shot carries `shot_type`, `angle`, `movement`, `content`, and `focus_on` from the camera directive.
