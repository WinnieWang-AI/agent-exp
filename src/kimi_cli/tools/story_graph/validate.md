...
- **Coverage**: every event has at least one location_state in `location_active_during`; every character that appears in an event's `interactions` or camera `focus_on` has a corresponding appearance active during that event.
