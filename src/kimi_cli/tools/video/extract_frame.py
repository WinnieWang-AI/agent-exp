...
class ExtractFrame(CallableTool2[Params]):
    ...
    async def __call__(self, params: Params) -> ToolReturnValue:
        ...
        if position == "first":
            cmd = self._build_first_frame_cmd(params.video_path, params.output_path)
        elif position == "last":
            duration = await self._get_duration(params.video_path)
            if duration is None:
                return builder.error(
            ...
            cmd = self._build_seek_cmd(params.video_path, params.output_path, seek)
        else:
            try:
                ts = float(position)
            except ValueError:
            ...
            cmd = self._build_seek_cmd(params.video_path, params.output_path, ts)
        ...
        if not Path(params.output_path).exists():
            return builder.error(
        ...
        builder.write(f"  source: {params.video_path}\n")
        builder.write(f"  position: {params.position}\n")
        builder.write(f"  output: {params.output_path}\n")
        return builder.ok(message=f"Frame saved to {params.output_path}")
